...
A durable, publishable record of something the data could not represent faithfully, kept in the project file beside the data it describes rather than in a log. The kinds today cover a completed hierarchy migration, a contested child, a hierarchy edge crossing two Entity kinds, an edge dropped by an identity merge, and an entry the file carried that could not be validated. Stated as a list rather than a count, because a glossary that names a number goes stale on the next addition. Each carries a one-sentence **summary written to be read** — naming entities, not ids — and a structured detail that captures the rejected link **verbatim at the moment of rejection**, unnormalised, because a derived value recomputes to nothing and the original pair would otherwise survive exactly one save. An analyst may acknowledge an event; an unacknowledged one blocks nothing, because refusing to save an analyst's only working file is the wrong direction to fail in.
_Avoid_: warning, error, log entry.

**Acknowledgement** (`acknowledgedBy` / `acknowledgedAt` / `acknowledgedNote`, ADR 0013):
A record that a **person has read** an integrity event. It asserts nothing about the world, which is what separates it from **Confirmation** — ADR 0009's gated human act that promotes a claim to credibility `1` — and why it carries none of that ceremony: free text, no eligibility gate, no second party. It never alters, closes or resolves the event it annotates, and it cannot be overwritten once made, since the fields are single-valued and a replacement would destroy the first attribution silently. `acknowledgedBy` is free text because Gabriel has no identity system and git supplies the real attribution.
_Avoid_: "confirm", "verify", "resolve", "dismiss", "clear" — an acknowledged event is **read**, not settled, and the panel says so in words.

**Absence notice** (`describeUnplacedByContest`, `core/map/unplacedNotice.ts`):
The statement that entities are missing from the map and why. A contested child derives no parent and therefore inherits no position, so it and everything below it are simply not drawn — the honest rendering under ADR 0011, which forbids inventing a midpoint or electing a winner. The notice is what keeps that honesty from reading as a silently short map. It names the count **and** the entities, and it covers descendants, since a reader told only about the contested entity would go looking for its children.
_Avoid_: "hidden", "filtered out", "error" — nothing was removed; a position was never derived.
