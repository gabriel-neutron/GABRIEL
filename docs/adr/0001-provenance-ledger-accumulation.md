...
- Wikipedia citations carry their own Authority Weight (used for retrieval confidence scoring) but are always excluded from ledger merges — both the accept-time top-2 selection and the independently-proposed-when-empty path — because Wikipedia is a tertiary source, not primary evidence for a claim.
- Ledger parsing, ranking, and merging logic is centralized in `services/enrichment/provenance-ledger.ts`; `enrichment.service.ts` (independent-proposal path) and `utils/enrichmentApply.ts` (accept-time merge) both delegate to it rather than reimplementing citation ranking.
- The only-when-empty invariant is enforced at request construction: `services/enrichment/request-builder.ts` (`buildEnrichmentRequest`) applies `provenanceLedger.shouldPropose` when building the output schema, and both orchestration paths (single-entity drawer and batch research) go through it. `runEnrichment` itself deliberately trusts the schema it receives — a hand-built request bypasses the gate, which is accepted because the builder is the only supported entry point.
